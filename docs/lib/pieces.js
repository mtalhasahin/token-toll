 ...
 * not.
 */
import { compile, segments } from "./regex.js";
 ...
 */
export function normalize(text, stage) {
    if (!stage)
        return text;
    switch (stage.type) {
        case 'Sequence': {
            const inner = (stage['normalizers'] ?? []);
        ...
        case 'Replace': {
            ...
            if (literal !== undefined)
                return text.split(literal).join(content);
            if (regex !== undefined)
                return text.replace(compile(regex), content);
            return text;
        }
        case 'Prepend':
...
export function pretokenize(text, stage) {
    if (!stage)
        return text.length > 0 ? [text] : [];
    switch (stage.type) {
        case 'Sequence': {
            const inner = (stage['pretokenizers'] ?? []);
            return inner.reduce((pieces, next) => pieces.flatMap((p) => pretokenize(p, next)), [text]);
        }
        case 'Split': {
