 ...
 */
export function expandInlineFlags(pattern) {
    const open = '(?i:';
    let out = '';
    let at = 0;
    for (;;) {
 ...
 */
export function segments(text, re, keepMatches, keepGaps) {
    const out = [];
    let last = 0;
    re.lastIndex = 0;
    for (let m = re.exec(text); m !== null; m = re.exec(text)) {
        if (keepGaps && m.index > last)
            out.push(text.slice(last, m.index));
        if (keepMatches && m[0].length > 0)
            out.push(m[0]);
